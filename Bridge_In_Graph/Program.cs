...
namespace Bridge_In_Graph
{
    class Main
    {
        ...
        private static void dfs(int node, int parent, int[] visited, int[] tin, int[] low, 
                                ref int timer, List<List<int>> adj)
        {
            visited[node] = 1;
            ...
            foreach (int it in adj[node])
            {
                ...
                if (visited[it] == 0)
                {
                    dfs(it, node, visited, tin, low, ref timer, adj);
                    low[node] = Math.Min(low[node], low[it]);

                    if (low[it] > tin[node])
                    {
                        Console.WriteLine(it + " --- " + node);
                    }
                }
                else
                {
        ...
        public void printBridges(int N, List<List<int>> adj)
        {
            ...
            for (int i = 0; i < N; i++)
            {
                if (visited[i] == 0)
                {
                    dfs(i, -1, visited, tin, low, ref timer, adj);
                }
            }
        }
    }
    ...
    class Program
    {
        ...
        {
            Console.WriteLine("Bridge_In_Graph");
            int N = 5;
